...
1. 用户发送消息。
2. 前端将会话 ID 和用户输入提交给后端。
3. 后端先将用户消息落库。
4. 后端根据会话历史拼接上下文。
5. 后端调用大模型流式接口生成回复。
6. 后端通过 SSE 持续推送增量结果给前端。
7. 生成结束后，将 assistant 回复落库。
...
- 创建成功后返回 `conversationId`、`title` 和 `userId`。
...
- URL：`GET /api/ai/conversations/{conversationId}`
- 功能：查询单个会话的基础信息
...
- URL：`GET /api/ai/conversations/{conversationId}/messages`
- 功能：分页查询会话消息记录
...
- URL：`GET /api/ai/chat/stop-status/{conversationId}`
- 功能：查询某会话是否已被停止
...
| 查询会话详情 | GET  | /api/ai/conversations/{conversationId}          | 获取会话基础信息   |
