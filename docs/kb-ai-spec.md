...
- 前端：React + Tiptap + 知识库列表 + 选区浮层 + 右键菜单 + 独立 AI 页面
...
#### 8. 独立 AI 页面 RAG 编排工作流

- 输入：会话消息 + 用户问题 + 可选文章范围 + 检索条数
- 处理：先查询 Redis Stack 向量数据库，再将命中的片段编入提示词，最后进入流式对话
- 输出：流式回答 + 引用列表 + 是否使用 RAG 标记
- 用途：让独立 AI 页面默认具备“先检索、后生成、带引用”的问答体验
...
- `heading`：作为章节边界，维护 `H1 > H2 > H3` 标题路径，并在 chunk 文本前注入 section 前缀（例如 `[Section] H1:系统设计 > H2:检索流程`）
- `paragraph`：作为正文基础单元，会与同一章节下的相邻内容合并为语义窗口
- `list` / `bulletList` / `orderedList`：列表项保留列表语义并并入当前章节，不按单条列表过细切分
- `codeBlock` / `blockquote`：保留块级语义，避免与普通段落无边界混合
- 切片采用“目标长度 + 句子边界优先”策略：目标约 1100 字符，最小约 500，最大约 1500；优先按 `。！？；` 或换行断句，必要时再按长度硬切
- 相邻 chunk 保留小范围重叠上下文（建议约 120 字符）以提升检索召回稳定性
- 当结构化切片结果为空时，才回退到纯文本按长度切片（建议每片约 1400 字符）
...
- 功能：基于文章向量索引做相似度召回，支持指定文章检索或当前用户全库检索，返回最相关的 chunk 列表
- 鉴权：需要登录
...
- 行为：
  - 进入 `/ai/:conversationId` 时，页面先按 `conversationId` 查询会话详情
  - 如果会话存在，则加载该会话的消息与引用，并保持当前路由不变
  - 如果会话不存在或查询失败，则创建一个新会话并跳转到新的会话路由
  - 进入 `/ai` 时，若当前没有有效会话，则自动创建新会话并跳转到对应路由
  - 切换到其他会话或新建会话时，浏览器地址栏始终同步当前 `conversationId`
...
- 行为：
  - 进入 `/knowledge-base/:articleId` 时，页面根据路由中的 `articleId` 选中并加载对应文章详情
  - 用户点击文章列表中的文章时，浏览器地址栏同步到对应的文章路由
  - 新建文章后，页面自动跳转到新文章对应的文章路由
  - 直接访问 `/knowledge-base` 时，若文章列表存在数据，则自动选中第一篇文章并同步到文章路由
- 刷新页面或从外部直接打开 `/knowledge-base/:articleId` 时，页面应优先恢复当前路由中的文章，不应因为列表重载而丢失选中状态

### 10.1 独立 AI 会话请求

- URL：`POST /api/ai/chat`
- 功能：独立 AI 页面默认先做 RAG 检索，再生成回答；返回回答正文、引用片段与是否命中 RAG
- 鉴权：需要登录
...
| useRag         | Boolean | 否  | 是否启用 RAG，默认 true  |
| articleId      | Long    | 否  | RAG 检索限定文章 ID      |
| topK           | Integer | 否  | 检索返回条数，默认 5       |
...
- 功能：独立 AI 页面流式输出，默认先完成 RAG 检索，再进行对话生成
...
- `rag-start` 表示服务端已经进入检索阶段
- `rag-result` 表示检索完成并返回引用列表；即使没有命中，也应返回 `citations: []`
- `message-start` 表示服务端已经创建好 AI 回复占位，并会附带本轮检索到的引用列表；即使没有命中，也应返回 `citations: []`
- `message-delta` 持续输出模型增量内容
- `message-end` 表示生成完成，并返回最终状态、消息 ID 与引用列表
- `message-stop` 表示用户主动停止生成，同样会返回已命中的引用列表
- `message-error` 表示流式生成失败
...
| 独立 AI 同步会话 | POST   | /api/ai/chat                            | 独立 AI 页面同步问答，默认先检索再生成      |
| 独立 AI 流式会话 | POST   | /api/ai/chat/stream                     | 独立 AI 页面流式问答，默认先检索再生成      |
| 停止独立 AI 生成 | POST   | /api/ai/chat/stop                       | 停止当前独立 AI 任务                |
